...
describe('Core.Utils', function() {
  ...
  describe('bbox', function() {
    it('should return false if input is not valid', function() {
      ...
      // invalid input
      let bbox = CoreUtils.bbox(new THREE.Vector3(), new THREE.Matrix3());
      expect(bbox).toEqual(false);

      // negative half dimensions
      bbox = CoreUtils.bbox(new THREE.Vector3(), new THREE.Vector3(-1, 10, 10));
    ...
    it('should return true if input is valid', function() {
      // zero bbox
      let bbox = CoreUtils.bbox(new THREE.Vector3(), new THREE.Vector3());
      //{ min: { x : 0, y : 0,  z : 0 },
      //  max: { x : 0, y : 0,  z : 0 }
      //}
