...
function editorSetup() {
      ...
      for(let j = ijkBBox[2]; j < ijkBBox[3] + 1; j++) {
        for(let k = ijkBBox[4]; k < ijkBBox[5] + 1; k++) {
          ...
          if(pixel[3] > 0 && i >= 0 && j >= 0 && k >= 0) {
            ...
            // update value...
            stack2.rawData[rawDataIndex][inRawDataIndex] = cursor.value;
  ...
  function onMouseMove(e) {
    ...
    context.beginPath();
    context.arc(currentPoint.x, currentPoint.y, cursor.size, false, Math.PI * 2, false);
    context.closePath();
    context.fill();
    context.stroke();
    ...
    let intersections = raycaster.intersectObject(stackHelper.slice._mesh);
    ...
    if(intersections.length > 0) {
      ...
      // use that to print pixel values
    }
  }
...
window.onload = function() {
  ...
  function buildGUI(stackHelper) {
    ...
    let brushSize = editorFolder.add(cursor, 'size', 1, 50).step(1);
    ...
    canvasDiv.addEventListener('contextmenu', function(e) {
