...
export default class SegmentationLutHelper {
  private _dom: HTMLElement;
  private _segmentation: { 
    0: { color: number[]; opacity: number; label: string; }; 
    1: { color: number[]; opacity: number; label: string; }; 
  };
  ...
  paintCanvas() {
    ...
    // TODO: Messy conversion from {for let...}
    for (let i = 0; i < 1; i++) {
