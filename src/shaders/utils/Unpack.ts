    ...
    private static computeDefinition(base, ) {
        ...
        if (base.uniforms.uNumberOfChannels.value === 1) {
          switch (base.uniforms.uBitsAllocated.value) {
            case 1:
            case 8:
              content = this.upack8(base);
              break;
    
            case 16:
              content = this.upack16(base);
              break;
    
            case 32:
              content = this.upack32(base);
              break;
        ...
        return  `
        void ${name}(in vec4 packedData, in int offset, out vec4 unpackedData){
